...
export function EgyptFarmMap({ highlightedFarmId }: { highlightedFarmId?: number | null }) {
  ...
  return (
    <div ref={mapRef}>
      ...
      {/* Map with Google Maps-style Background */}
      <div className="relative rounded-xl overflow-hidden" style={{ height: '600px' }}>
        {/* Map Background - Embedded OpenStreetMap */}
        <iframe
          src="https://www.openstreetmap.org/export/embed.html?bbox=24.0,22.0,37.0,32.0&layer=mapnik"
          style={{
            position: 'absolute',
            top: 0,
            left: 0,
        ...
        {/* Decorative elements */}
        <div className="absolute top-4 right-4 text-xs text-gray-500 bg-white/90 backdrop-blur-sm px-3 py-1.5 rounded shadow-sm z-20">
          {farms.length} Farms
        </div>
      </div>
