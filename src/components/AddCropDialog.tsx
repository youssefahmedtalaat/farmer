...
export function AddCropDialog({ open, onOpenChange, onCropAdded }: AddCropDialogProps) {
  const { t } = useLanguage();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Save to database
      await cropsApi.add(cropData);

      // Log activity
      await activitiesApi.log({
        action: 'Crop added',
        detail: `${cropData.name} - ${cropData.quantity}`,
      });

      // Show success notification
      notify.crop.added(cropData.name);
      ...
      if (error.message && (error.message.includes('subscription') || error.message.includes('trial') || error.message.includes('expired'))) {
        ...
        notify.error(t('addCrop.error.failed'), error.message || t('addCrop.error.failed'));
      }
    } finally {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 py-4">
          {/* Crop Name */}
          <div>
            <Label htmlFor="cropName">{t('addCrop.cropName')} {t('addCrop.required')}</Label>
            <Input
              id="cropName"
          ...
          {/* Quantity */}
          <div>
            <Label htmlFor="quantity">{t('addCrop.quantity')} {t('addCrop.required')}</Label>
        ...
        <div className="flex gap-3">
          ...
          >
            {isSubmitting ? t('addCrop.adding') : t('addCrop.addButton')}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
